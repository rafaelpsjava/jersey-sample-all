package br.com.sample.ejb.util.impl;
 ...
 * ServiceLocator
 * 
 * java:global/app-name/ java:app/app-name/beanToLookup java:module/beanToLookup
 * java:global/app-name/beanToLookup java:app/app-name/beanToLookup
...
public class ServiceLocator {
	...
	public synchronized Object findEjb(String beanName, Properties jndiProperties) {
		String[] beansToLookup = new String[] { String.format("java:module/%s", beanName), String.format("java:comp/env/ejb/%s", beanName) };
		for (String beanToLookup : beansToLookup) {
			try {
				Object foundEjb = new InitialContext(jndiProperties).lookup(beanToLookup);
				if (foundEjb != null) {
					return foundEjb;
				}
			} catch (Throwable throwable) {
